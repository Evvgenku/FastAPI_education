...
async def new_access_token(body: RefreshTokenRequest, db: AsyncSession = Depends(get_async_db)):
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Could not validate refresh token",
        headers={"WWW-Authenticate": "Bearer"},
...
@router.put('/{user_id}')
async def make_admin(user_id: int, db: AsyncSession = Depends(get_async_db)):
    user = await db.scalars(select(UserModel).where(UserModel.is_active == True, UserModel.id == user_id))
    ...
    return {'message': 'Users role is updated'}